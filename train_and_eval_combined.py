...
COCO_ROOT = '../Train_Data/images'
COCO_ANN_TRAIN = '../Train_Data/captions_val2017.json'
COCO_ANN_VAL = '../Train_Data/captions_val2017.json'
...
class FlamingoBaseModel(ABC, PreTrainedModel):
    ...
    def __init__(self, config: FlamingoConfig, suppress_warnings=True):
        ...
        with suppress_model_loading_warnings(suppress_warnings):
            self.vision_encoder = CLIPVisionModel.from_pretrained(
                config.clip_model_type)  # type: ignore
    ...
    def encode_resample_visuals(self, pixel_values: torch.Tensor) -> torch.Tensor:
        ...
        visual_features = rearrange(
            visual_features, '(b N T) v d -> (b N) T v d', b=b, N=N, T=T)
        visual_features = self.resampler(visual_features)

        # T is gone at this point
        visual_features = rearrange(
            visual_features, '(b N) q d -> b N q d', b=b, N=N)
    ...
    def forward(
        ...
        **kwargs
        ...
        assert (input_ids is None) != (
            inputs_embeds is None), "you must pass either input_ids or inputs_embeds!"

        # find the input shape
        batch_size, seq_length = input_ids.shape[:
                                                 2] if input_ids is not None else inputs_embeds.shape[:2]
        ...
        if visual_features is None:
            if xattn_past_key_values is None and pixel_values is not None:
                ...
                visual_features = torch.zeros(
                    (batch_size, 1, self.config.resampler_num_latents,
                     self.config.dim_visual),
                    dtype=torch.float32,
                    device=device
                )

        if media_locations is None:
            media_locations = torch.zeros(
                size=(batch_size, seq_length), dtype=torch.int, device=device)
        ...
        return CausalLMOutputWithPast(
            loss=loss,
            logits=logits,
            past_key_values=(tuple(xattn_past_key_values),
                             out.past_key_values) if use_cache else None,
...
class FlamingoGPT2(FlamingoBaseModel):
    ...
    def __init__(self, config: FlamingoConfig):
        ...
        base_lm: GPT2LMHeadModel = GPT2LMHeadModel.from_pretrained(
            config.lm)  # type: ignore
    ...
    def generate_captions(
        ...
        **kwargs
        ...
        if images is not None:
            ...
            pixel_values = processor(images=images, device=device)[
                'pixel_values']
    ...
    def score_sequences(
        ...
        out = self.flamingo(
            ...
            pixel_values=pixel_values.unsqueeze(
                0) if pixel_values is not None else None,           # add batch_size of 1
            visual_features=visual_features.unsqueeze(
                0) if visual_features is not None else None,  # add batch_size of 1
            use_cache=True,
        )
...
class MaskedCrossAttention(nn.Module):
    ...
    def forward(
        ...
        if previous_kv is None:
            ...
            visual_features = rearrange(
                visual_features, 'b t n d -> b (t n) d')

            k, v = self.to_kv(visual_features).chunk(2, dim=-1)
            q, k, v = rearrange_many(
                (q, k, v), 'b n (h d) -> b h n d', h=n_heads)
        else:
        ...
        text_to_media_mask = rearrange(
            text_time, 'b i -> b 1 i 1') == repeat(media_time, 'j -> 1 1 1 (j m)', m=self.n_visual)
...
class GatedCrossAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        attn_out, kv = self.attn(
            y, media_locations, visual_features, previous_kv=previous_kv, output_kv=output_kv)
...
def evaluate_image_captioning(
    ...
    *,
    ...
    wrapper = Subset(wrapper, range(
        start, end if end is not None else len(wrapper)))
    loader = DataLoader(
...
class FlamingoTrainingArguments(TrainingArguments):
    """ custom arguments """
    eval_coco_captioning_prefix: str = field(
        default="<image>A picture of")         # It's a common thing to do for COCO image captioning
...
class FlamingoProcessor:
    ...
    def __init__(
        ...
        if config.lm.startswith('gpt2'):
            ...
            self.tokenizer = AutoTokenizer.from_pretrained(
                'facebook/opt-30b', use_fast=use_fast)
    ...
    def __call__(
        ...
        if images is not None:
            result['pixel_values'] = self.vision_processor(
                images=images, return_tensors='pt', padding=True)['pixel_values'].to(device)

        if text is not None:
            input_ids, media_locations, attention_mask = self.encode_text(
                text, device=device)
